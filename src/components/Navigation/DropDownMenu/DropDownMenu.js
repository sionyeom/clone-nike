...
const DropDownMenu = observer(({ navListNum, setNavListNum, setIsDropDownHover}) => {
  ...
  useEffect(() => {
    let getNavListNum = navListNum ? navListNum : 0;
  ...
  return (
    <>
    {navListNum === 0 ? console.log(0) : <div className="dropdown_wrapper" onMouseOver={() => setIsDropDownHover(1)} onMouseOut={() => setIsDropDownHover(0)}>
      {hoveredList.map(e => {
                ...
                if(e.undertitle &&  underOptions.length !== 0 ) {
                    ...
                    return (<>
                        <div className="dropdown_box" key={e}>
                            ...
                            {optionArr.map((el_1) => {
                                return (
                                    <><div className="dropdown_option" key={el_1}>{el_1}</div></>)})}
                        
                            <div className="dropdown_title under" >{e.undertitle}</div>
                            {underOptions.map((el_2) => {
                                return (
                                    <><div className="dropdown_option" key={el_2}>{el_2}</div></>)})}
                        </div>
                    </>)
                }else {
                    // 아래 옵션이 없을 경우
                    ...
                    <div className="dropdown_box" key={e}>
                        <div className="dropdown_title">{e.title}</div>
                        {optionArr.map((e) => {
                            return (<><div className="dropdown_option" key={e}>{e}</div></>)})}</div></>)                         
                    }
            })}
        </div>
      </>)
